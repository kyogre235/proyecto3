...
public class ArbolBinarioOrdenado<T extends Comparable<T>>
    ...
    @Override public void agrega(T elemento) {
        ...
        Vertice vnuevo = nuevoVertice(elemento);
        elementos++;
        ...
        ultimoAgregado = vnuevo;
    }

    //metodo recursivo para agregar
    private void agrega (Vertice actual, Vertice vnuevo){
       
        if(vnuevo.elemento.compareTo(actual.elemento) <= 0){
           
            if(actual.izquierdo == null){
                actual.izquierdo = vnuevo;
                vnuevo.padre = actual;
            } else 
                agrega(actual.izquierdo, vnuevo);
            
        } else {
           
            if(actual.derecho == null){
                actual.derecho = vnuevo;
                vnuevo.padre = actual;
            } else 
                agrega(actual.derecho, vnuevo);
    ...
    protected Vertice intercambiaEliminable(Vertice vertice) {
        ...
        Vertice vmax = maxvertice(vertice.izquierdo);
        T pivote = vmax.elemento;

        vmax.elemento = vertice.elemento;
        vertice.elemento = pivote;

        return vmax;

    }

    // forma recursiva de encontrar el maximo vertice del lado izq
    private Vertice maxvertice (Vertice v){
        if(v.derecho == null)
            return v;
        
        return maxvertice(v.derecho);
    ...
    protected void eliminaVertice(Vertice vertice) {
        ...
        if(vertice.padre == null)
            ...
            if(vertice.padre.izquierdo == vertice)
                vertice.padre.izquierdo = h;
            else
                vertice.padre.derecho = h;
        }
       if(h != null)
            h.padre = vertice.padre;
    }
    ...
    private VerticeArbolBinario<T> buscaAux(T elemento, Vertice vertice){
        if(vertice == null)
            return null;
        ...
        if (elemento.compareTo(vertice.elemento) <= 0)
            return buscaAux(elemento, vertice.izquierdo);
        else
            return buscaAux(elemento, vertice.derecho);
    }
